...
# def get_rolling_avgs(player_df):
    ...
    player_df['PTS_5_game_trend'] = player_df['PTS'].rolling(5).mean().diff()
    ...
    player_df['PTS_pct_of_max'] = player_df['PTS_last_5_avg'] / (player_df["PTS"].max())
...
#         pts_avg/player["PTS"].max()
#     )
#     pts_trend = np.polyfit(
#         x=np.arange(len(last_10['PTS_5_MA'].dropna())),  # x = time steps
#         y=last_10['PTS_5_MA'].dropna(),                  # y = moving avg values
#         deg=1                                            # linear fit
#     )[0]  # [0] = slope, [1] = intercept
...
def prep_game(opponent, player, teams):
    ...
    pts_volatility = last_10['PTS'].rolling(window=10).std().iloc[-2]  # shift(1)
